...
/**
 * Screen-only hairlines where the printed sheets will be cut. The flow is one
 * continuous column on screen, so without them there is no way to tell which
 * paragraph is about to land on the next page. On screen the band padding is
 * drawn once, at the top, which is why the offset is added only once.
 */
function PageBreakGuides({ pageCount }: { pageCount: number }) {
    ...
    <div aria-hidden className="no-print pointer-events-none absolute inset-0">
      {Array.from({ length: pageCount - 1 }).map((_, index) => (
        <div
          ...
          style={{
            top: `calc(${PAGE_MARGIN_TOP_MM}mm + ${index + 1} * ${PAGE_CONTENT_HEIGHT_MM}mm)`,
          }}
        >
          <div className="w-[32%] border-t border-dashed border-white/40" />
          <div className="w-[68%] border-t border-dashed border-gray-400" />
        </div>
      ))}
    </div>
  );
...
export function ClassicTealTemplate({
  ...
  return (
    ...
    >
      {mode !== "thumbnail" && <PageBreakGuides pageCount={pageCount} />}

      <aside className="w-[32%] bg-[#124355] text-white">
        <div className="resume-band px-8">
          <div ref={sidebarRef} className="flex flex-col gap-8">
            ...
            {view.skillGroups.length > 0 && (
              <div className="flex flex-col gap-6">
                ...
                {/* Only the title + bar of each group is unbreakable, and it is
                  welded to the first skill. The list itself flows, so a long
                  group keeps filling the current page and spills the overflow
                  onto the next one instead of being moved whole and leaving a
                  teal hole behind it. */}
                {view.skillGroups.map(group => (
                  <div key={group.key}>
                    <div className="resume-block resume-keep-next">
      ...
      <main className="w-[68%] text-gray-800">
        <div className="resume-band px-10">
          <div ref={mainRef} className="flex flex-col gap-10">
            {view.summary && (
              <section>
                <div className="mb-4">
